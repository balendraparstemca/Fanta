...
// let iteration = 0; // gets iterated when we scroll all the way to the end or start
...
//     },
//     seamlessLoop = buildSeamlessLoop(cards, spacing, animateFunc),
//     playhead = {offset: 0}, // proxy object to simulate playhead position
//     wrapTime = gsap.utils.wrap(0, seamlessLoop.duration()), // ensures safe time wrapping
//     scrub = gsap.to(playhead, {
//         offset: 0,
//         onUpdate() {
//             seamlessLoop.time(wrapTime(playhead.offset)); // converts offset to a "safe" time
//         },
//         duration: 0.5,
//         ease: "power3",
//         paused: true
...
//             // Prevent wrapping at extremes
//             if (scroll > self.end - 1 || (scroll < 1 && self.direction < 0)) {
//                 return; // Stop further handling when at the scroll boundaries
//             }

//             scrub.vars.offset = (iteration + self.progress) * seamlessLoop.duration();
//             scrub.invalidate().restart(); // Improve performance by reusing the tween
//         },
//         end: "+=3000",
//         markers: true,
//         pin: ".gallery"
...
// // Snap to the closest item when the user stops scrolling
// ScrollTrigger.addEventListener("scrollEnd", () => scrollToOffset(scrub.vars.offset));

// // Moves the scroll playhead to the correct place and wraps if needed
